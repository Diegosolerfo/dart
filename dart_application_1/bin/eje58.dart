...
void main(){
  ...
  print("Digite la tasa de interes (en %): ");
  double tasaInteres = double.parse(stdin.readLineSync()!);
  print("Digite la duración del deposito en semanas: ");
  int duracionSemanas = int.parse(stdin.readLineSync()!);

  double tasaDiaria = tasaInteres / 100 / 365;
  int dias = duracionSemanas * 7;
  double capitalAcumulado = capital;

  for(int i=0; i<dias; i++){
    double interesDiario = capitalAcumulado * tasaDiaria;
    capitalAcumulado += interesDiario;
  }

  print("El capital total acumulado al final del periodo es: \$${capitalAcumulado.toStringAsFixed(2)}");
}